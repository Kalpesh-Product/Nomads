...
const recommendationCards = [
  {
    title: "BASIC",
    subtitle:
      "Everything you need to start, manage, and grow your business at no cost!",
    price: "FREE*",
    ctaText: "Get Started",
    highlight: false,
...
const AiHostPricing = ({ compact = false, startStep = 1, onSelectPlan }) => {
  ...
  const fourthLineText =
    "Choose your goals from below so that we can become your extended team for your business.";
  ...
  const handleCardClick = (card) => {
    ...
    navigate(targetPath, {
      state: card.path.includes("/search")
              ...
              {recommendationCards.length > 3 && (
                <>
                  <button
                    ...
                    className={`absolute -left-5 top-1/2 z-10 -translate-y-1/2 rounded-full border bg-white p-3 shadow-md transition-all ${
                      canScrollLeft
                  ...
                  <button
                    ...
                    className={`absolute -right-5 top-1/2 z-10 -translate-y-1/2 rounded-full border bg-white p-3 shadow-md transition-all ${
                      canScrollRight
                        ? "border-[#d9e2f0] text-[#1e2b45] hover:bg-white"
                        : "cursor-not-allowed border-[#e7edf5] text-[#a9b4c8]"
                    }`}
              ...
              >
                {recommendationCards.map((card, index) => {
                  ...
                  return (
                    ...
                    >
                      <article
                        className={`relative flex h-full flex-col rounded-[34px] border bg-[#f5f7fb] p-6 text-left transition-all duration-300 sm:p-7 ${
                          selectedPlanTitle === card.title
                      ...
                      >
                        ...
                        <div className="mt-6 flex items-baseline justify-center gap-2">
                          <span className="text-2xl font-extrabold text-[#0f1730]">
                            {card.price}
                          </span>
                          {card.priceSuffix && (
                            <span className="text-lg font-semibold text-[#60708b]">
                              {card.priceSuffix}
                            </span>
                          )}
                        </div>
                        ...
                        <ul className="flex flex-1 flex-col gap-3">
                          {card.features.map((feature) => {
                            ...
                            return (
                              <li
                                key={feature}
                                className="flex items-center gap-3 text-[#5a6d89]"
                              >
                        ...
                        <div className="mt-7">
                          ...
                          <button
                            ...
                            className={`w-full rounded-full px-4 py-3 text-base font-medium transition-colors ${
                              card.highlight
