...
// "Fresh Studio" — dark, image-forward theme ported from HostPanel's
// FreshStudioTemplate.tsx (client/src/pages/Dashboard/FrontendDashboard/
// WebsiteBuilder/templates/FreshStudioTemplate.tsx): near-black background
// (#0A0A12) with white text and a soft red accent (#D94B4B), small precise
// radii with pill buttons, tight functional spacing. This module holds the
// visual atoms shared by all 9 Fresh Studio section pages plus the
// header/footer — mirrors HostPanel's inline sub-components 1:1 so the
// visuals stay byte-equivalent, only the data plumbing changed (Nomads field
// names / real navigation instead of HostPanel's preview-only state).
...
// A home-level productDropdownPage's card description arrives under
// different keys depending on shape — check all of them so the card never
// silently drops it.
export const getProductCardDescription = (product) =>
  String(product?.subText || product?.homeCardSubText || product?.description || "").trim();

export const getProductCardImage = (product, fallbackImage) => {
  const candidate =
    product?.cardImage ?? product?.images?.[0] ?? product?.heroImage ?? product?.heroImages?.[0] ?? "";
  const resolved = typeof candidate === "string" ? candidate : candidate?.url || candidate?.preview || "";
  return resolved || fallbackImage || "";
};

export const ProductGrid = ({ products, onSelect, fallbackImage }) => (
      ...
      return (
        ...
        >
          ...
          <div className="flex flex-1 flex-col items-center gap-1.5 p-5 text-center">
            ...
            <span className="mt-2 text-[12px] font-semibold" style={{ color: ACCENT }}>
              View details →
            </span>
          </div>
        </button>
      ...
      <div className="mt-7 overflow-hidden">
        <div className="flex items-center justify-center gap-8 md:gap-16">
          {displayed.map((src, idx) => (
            ...
            >
              <img src={src} alt={`Partner logo ${idx + 1}`} className="max-h-full max-w-full object-contain" />
            </div>
          ))}
        </div>
      </div>
...
// items to the end of the track. Falls back to a static grid when there
// aren't enough testimonials to fill more than one page.
      ...
      <div className="overflow-hidden">
        ...
        >
          {extended.map((item, idx) => (
            <div key={`${item?.key || item?.name}-${idx}`} className="shrink-0 px-2.5" style={{ width: `${slideWidth}%` }}>
...
// paragraph text, same heuristic Nomads' own FaqAccordion.jsx uses.
            ...
            <div key={idx} className={`${CARD} overflow-hidden border`} style={{ borderColor: "rgba(255,255,255,0.12)" }}>
              ...
              >
                <span className="flex min-w-0 items-center gap-3">
                  ...
                  <span className="text-[14px] font-semibold md:text-[15px]" style={{ color: HEADING }}>
                    {faq.question}
                  </span>
                </span>
                <span
                    ...
                    {blocks.map((block, bi) =>
                      block.type === "bullet" ? (
                        <div key={bi} className="flex items-start gap-2">
                          ...
                          <p className="text-[13px] leading-relaxed md:text-[14px]" style={{ color: MUTED }}>
                            {block.text}
                          </p>
                        </div>
                      ) : (
                        <p key={bi} className={`text-[13px] leading-relaxed md:text-[14px] ${hasBullets ? "pl-4" : ""}`} style={{ color: MUTED }}>
                          {block.text}
                        </p>
                      ),
                    )}
...
// Same master inclusions icon set as Nomads' own InclusionsSection.jsx /
// HostPanel's inclusionIcons.tsx (kept in sync across all three repos) —
// duplicated here (rather than imported) so this stays ACCENT-colorable and
// Fresh-Studio-specific without touching the shared light-themed component
// Classic already depends on.
