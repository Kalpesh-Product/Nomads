...
import { NomadsSiteContext } from "./NomadsSiteContext";
...
// Published data stores media as { id, url } objects; the templates read plain URLs for these
// (the builder preview draft already has them as strings).
const normalizeSiteMedia = (data) => ({
    ...data,
    companyLogo: getMediaSrc(data?.companyLogo || data?.companyLogoUrl),
    mainHeroImage: getMediaSrc(data?.mainHeroImage),
    heroImages: (Array.isArray(data?.heroImages) ? data.heroImages : []).map((item) => getMediaSrc(item)).filter(Boolean),
    aboutPageImageCards: (Array.isArray(data?.aboutPageImageCards) ? data.aboutPageImageCards : []).map((card) => ({ ...card, image: getMediaSrc(card?.image) })),
    founders: (Array.isArray(data?.founders) ? data.founders : []).map((founder) => ({ ...founder, image: getMediaSrc(founder?.image) })),
});
...
export const isMenuProductSlug = (slug) => {
    const normalized = normalizeSlug(slug);
...
const mapReviewToTestimonial = (item) => ({
    ...
    name: String(item?.reviewerName || item?.reviewreName || item?.fullName || item?.name || "").trim() ||
        "Reviewer",
...
export const FOOTER_SOCIAL_KEYS = ["instagram", "facebook", "twitter", "linkedin", "whatsapp"];
...
const isUsableSocialHref = (key, href) => {
    if (!href)
        return false;
    if (key === "whatsapp")
        return /^https:\/\/wa\.me\/\d{7,}$/.test(href);
    try {
...
export const getCareersJobTitle = (job) => String(job?.title || job?.designation || job?.name || "Untitled Role").trim();
const formatCareersMetaValue = (value, mode = "generic") => {
    ...
    if (mode === "workMode") {
        if (raw === "on_site" || raw === "onsite" || raw === "on-site" || raw === "on site")
            return "ON-SITE";
        if (raw === "remote")
            return "REMOTE";
        if (raw === "hybrid")
...
export const getCareersJobMeta = (job) => {
    const meta = [
...
export const getProductContentItems = (draft, slug, page) => {
    const normalized = normalizeSlug(slug);
    if (normalized.includes("meeting")) {
...
export const useWebsiteTemplateData = () => {
    ...
    // On a hosted site the data comes from the page that fetched it, not from a builder preview draft.
    const site = useContext(NomadsSiteContext);
    const [draft, setDraft] = useState(site?.data ? normalizeSiteMedia(site.data) : null);
    ...
    useEffect(() => {
        if (site)
            setDraft(site.data ? normalizeSiteMedia(site.data) : null);
    }, [site?.data]);
    useEffect(() => {
        if (site)
            return undefined;
    ...
    const sourceNavItems = useMemo(() => Array.isArray(draft?.pageNavItems)
        ? draft.pageNavItems
    ...
    const navItems = useMemo(() => {
        const fromDraft = sourceNavItems
            ...
            const name = slug === "products" && (!rawName || rawName.toLowerCase() === "products")
                ? "Services"
                : rawName || "Page";
            return { name, slug };
        });
    ...
    const isNavPageEnabled = useCallback((slug) => {
    ...
    const productPages = useMemo(() => {
        ...
        products.forEach((p) => {
            ...
            [normalizeSlug(p?.type), normalizeSlug(p?.name)].filter(Boolean).forEach((s) => {
                if (!productImageBySlug[s])
                    productImageBySlug[s] = url;
            });
        });
        const resolveCardImage = (item, index) => getMediaSrc(item?.cardImage) ||
            getMediaSrc(item?.homeCardImage) ||
            getMediaSrc((Array.isArray(item?.heroImages) ? item.heroImages : [])[0]) ||
            getMediaSrc(item?.heroImage) ||
            getMediaSrc((Array.isArray(item?.subProducts) ? item.subProducts : []).find((sp) => sp?.enabled !== false && getMediaSrc(sp?.images?.[0]))?.images?.[0]) ||
        ...
        if (dropdownPages.length > 0) {
            return dropdownPages
                .filter((item) => item?.enabled !== false)
                .map((item, index) => ({ ...item, cardImage: resolveCardImage(item, index) }));
        }
    ...
    const selectedProductPage = useMemo(() => productPages.find((item) => normalizeSlug(item?.slug || item?.name || "") === currentProductSlug) || null, [productPages, currentProductSlug]);
    const selectedDetailItem = useMemo(() => {
        if (!currentItemSlug || !selectedProductPage)
            return null;
    ...
    const selectedProductHeroImage = getMediaSrc(selectedProductHeroImages[productHeroIndex]) ||
    ...
    const fetchPostedJobs = useCallback(async () => {
        ...
        try {
            const response = await api.get(`/editor/get-jobs/${workspaceId}`);
            const list = response?.data?.data || response?.data?.jobs || response?.data;
            setCareersJobs(Array.isArray(list) ? list : []);
        }
        catch (err) {
            setCareersJobs([]);
        }
    ...
    useEffect(() => {
        const fetchApprovedReviews = async () => {
            ...
            try {
                const response = await api.get("/review/approved", { params: { companyId, source: "website" } });
                setApprovedReviews(Array.isArray(response?.data?.data) ? response.data.data : []);
            }
            catch (error) {
    ...
    const galleryItems = Array.isArray(draft?.gallery)
        ? draft.gallery.filter((item) => item?.enabled !== false).map((item) => getMediaSrc(item)).filter(Boolean)
        : [];
    ...
    const testimonials = [...draftTestimonials, ...approvedTestimonials].filter((item, index, array) => index ===
        array.findIndex((candidate) => String(candidate?.key || "").trim() === String(item?.key || "").trim() ||
    ...
    useEffect(() => {
        const onResize = () => {
            if (window.innerWidth < 768)
                setTestimonialPerView(1);
            else if (window.innerWidth < 1024)
                setTestimonialPerView(2);
            else
                setTestimonialPerView(3);
        };
        window.addEventListener("resize", onResize);
        return () => window.removeEventListener("resize", onResize);
    }, []);
    const aboutBlocks = Array.isArray(draft?.about) ? draft.about : [];
    const aboutIntroBlocks = getNonEmptyTextList(draft?.aboutPageIntro, draft?.aboutPageOverview, ...aboutBlocks.map((block) => (typeof block === "string" ? block : block?.text)));
    ...
    const aboutPageImageCards = Array.isArray(draft?.aboutPageImageCards)
        ? draft.aboutPageImageCards.filter((card) => String(card?.title || "").trim() || String(card?.description || "").trim() || card?.image)
        : [];
    ...
    const getPreviewRouteForSection = (slug) => {
        ...
        return sectionId === "home" ? "/page/home" : `/page/${sectionId}`;
    };
    const getPreviewRouteForProduct = (slug) => `/page/products/${normalizeSlug(slug)}`;
    ...
    const goToProductItem = (productSlug, itemSlug) => {
        navigate(`/page/products/${normalizeSlug(productSlug)}/${normalizeSlug(itemSlug)}`);
    };
    const handleProductCardAction = (product) => {
        const slug = normalizeSlug(product?.slug || product?.name || "");
        navigate(`/page/products/${slug}`);
    };
    ...
    const handleProductHeroNext = () => {
        if (selectedProductHeroImages.length <= 1)
            return;
    ...
    const submitReviewForm = async (event) => {
        ...
        setReviewSubmitError("");
        try {
            await api.post("/review/website-review", {
                companyId: draft?.companyId || "",
                businessId: draft?.companyId || "",
                companyName: draft?.companyName || "",
                name: reviewForm.reviewerName?.trim() || "Anonymous",
                starCount: Number(reviewForm.rating || 5),
                description: reviewForm.review?.trim() || "",
                workspaceId: draft?.workspaceId || "",
                reviewSource: "Website Reviews",
                reviewLink: window.location.href,
            });
    ...
    const submitCareersApplication = async (event) => {
        ...
        try {
            ...
            await axios.post(`${masterPanelBaseUrl}/api/recruitment/jobs/apply`, payload);
