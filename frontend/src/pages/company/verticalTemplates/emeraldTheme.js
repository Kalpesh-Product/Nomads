...
const toHex = ({ r, g, b }) => `#${[r, g, b]
    ...
    .join("")}`;
export const mix = (a, b, weightOfB) => {
    const x = toRgb(a);
    const y = toRgb(b);
    return toHex({
...
export const hasCustomEmeraldTheme = (config) => Boolean(normalizeHex(config?.bgColor) ||
...
export const resolveEmeraldTheme = (config) => {
...
export const EMERALD_BASE_CSS = ".es-scope .text-white\\/60{color:rgba(255,255,255,0.9)}.es-scope .text-white\\/40{color:rgba(255,255,255,0.85)}";
const esc = (cls) => cls.replace(/([^a-zA-Z0-9_-])/g, "\\$1");
export const buildEmeraldThemeCss = (config, scope = ".fm-template") => {
    if (!hasCustomEmeraldTheme(config))
        return "";
    ...
    [10, 15, 20, 25, 40, 50, 60].forEach((n) => add(`border-white/${n}`, `border-color:${rgba(text, n / 100)}`));
    ...
    add("via-[#4a6b96]", `--tw-gradient-to:${rgba(bg, 0)} var(--tw-gradient-to-position);--tw-gradient-stops:var(--tw-gradient-from),${bg} var(--tw-gradient-via-position),var(--tw-gradient-to)`);
