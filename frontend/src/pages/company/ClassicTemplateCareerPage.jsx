...
const FIELD_CLASS =
  "w-full rounded-lg border border-slate-300 px-3 py-2 text-[13px] outline-none focus:border-[#111827]";
const SELECT_CLASS =
  "w-full rounded-lg border border-slate-300 px-3 py-2 text-[13px] outline-none focus:border-[#111827] appearance-auto";

const ClassicTemplateCareerPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append(
        "phone",
        applyDialCode && form.phone
          ? `${applyDialCode} ${form.phone.trim()}`
          : form.phone,
      );
  ...
  const careersClosingHeading = String(
    data?.careersClosingHeading || "",
  ).trim();
  const applyBtnText =
    String(data?.careersApplyButtonText || "").trim() || "Apply Now";

  const introParagraphs = careersPageIntro
    ? careersPageIntro.split("\n").filter(Boolean)
    : CAREERS_FALLBACK_INTRO;

  const CAREERS_PAGE_HEADING = companyName
    ? `Join Our Team - ${companyName}`
    : "Join Our Team";
      ...
      <div className="mx-auto mt-6 w-full max-w-7xl">
        ...
        <div className="mx-auto mt-10 max-w-4xl text-center text-[15px] leading-[1.9] text-[#374151] font-['Poppins',ui-sans-serif,system-ui,sans-serif] md:text-[17px]">
          {introParagraphs.map((para, i) => (
            <p key={`intro-${i}`} className="mb-5 last:mb-0">
              {para}
            </p>
          ))}
        </div>
        ...
        {jobsLoading ? (
          ...
          <div className="mt-10 rounded-[28px] px-6 py-10 text-center ">
            ...
            <p className="mt-2 text-[14px] text-[#374151]">
              Please check back later or use the apply button to share your
              resume.
            </p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <button
                    type="button"
                    onClick={() =>
                      setOpenDept(isOpen ? "" : section.department)
                    }
...
export default ClassicTemplateCareerPage;
