...
const normalizeImageList = (...sources) => {
  const urls = sources
    .flatMap((source) => {
      ...
      if (typeof source === "object") {
        return source.url || source.image || source.src || "";
      }
  ...
  return Array.from(new Set(urls)).map((url, index) => ({
    _id: `restaurant-image-${index}`,
    url,
  }));
};

const normalizeRestaurantReviews = (reviews) => {
  if (!Array.isArray(reviews)) return [];

  return reviews.map((review, index) => ({
    _id: review._id || `restaurant-review-${index}`,
    ...review,
...
const normalizeRestaurantPoc = (poc) => {
  if (!poc || typeof poc === "string") return {};

  return {
    name: poc.name || poc.fullName || "",
    designation: poc.designation || poc.role || "",
  };
};

const normalizeRestaurantDetails = (restaurant = {}) => {
  ...
  return {
    ...restaurant,
    companyId:
      restaurant.restaurantId || restaurant.businessId || restaurant._id,
    businessId:
      restaurant.businessId || restaurant.restaurantId || restaurant._id,
    companyName: restaurantName,
    companyTitle: restaurant.restaurantTitle || restaurantName,
    companyType: "restaurants",
    websiteTemplateLink: restaurant.website || "",
    ...
    about:
      ...
      "",
    ratings: Number.isFinite(rating) ? rating : 0,
    totalReviews: restaurant.totalReviews || 0,
    reviewCount: restaurant.totalReviews || 0,
    reviews: normalizeRestaurantReviews(restaurant.reviews),
    inclusions:
      ...
      "",
    poc: normalizeRestaurantPoc(restaurant.poc),
    isLiked: false,
  };
};

const AiRestaurantProduct = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const { company, restaurantId } = useParams();
  ...
  const isRestaurantDetailPage = Boolean(restaurantId);
  ...
  const { data: companyDetails, isPending: isCompanyDetails } = useQuery({
    queryKey: [
      isRestaurantDetailPage ? "restaurantDetails" : "companyDetails",
      restaurantId || null,
      companyId,
      companyName || "unknown",
      userId || "guest",
    ...
    queryFn: async () => {
      if (isRestaurantDetailPage) {
        ...
        return normalizeRestaurantDetails(response?.data);
      }
    ...
    enabled: isRestaurantDetailPage
      ? !!restaurantId
  ...
  const companyImages = useMemo(
    () => companyDetails?.images?.slice(0, 4) || [],
    [companyDetails?.images],
  );
        ...
        w-full md:w-3/4 lg:w-1/2  shadow-lg px-6 py-3 undefined"
            ...
            <div className="text-[0.74rem] text-gray-500 leading-relaxed">
              ...
              >
                ...
                <p className="mb-2">
                  WoNo does not claim ownership of any third-party logos,
...
export default AiRestaurantProduct;
