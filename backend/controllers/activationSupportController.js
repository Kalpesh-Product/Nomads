...
import ActivationSupportPartner from "../models/ActivationSupportPartner.js";
...
const readActivationSupportPartnerRows = (file) =>
...
const buildActivationSupportPartnerFromRow = (row) => {
...
const editableActivationSupportPartnerSchema = yup.object({
  srNo: nullableNumber(),
...
const buildEditableActivationSupportPartnerPayload = async (body = {}) => {
  const payload = await editableActivationSupportPartnerSchema.validate(body, {
    abortEarly: false,
    stripUnknown: true,
  });
...
export const importActivationSupportPartnersCsv = async (req, res, next) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        message:
          "Please upload a CSV file using form-data field activation-support-partners-file.",
      });
    }

    const rows = await readActivationSupportPartnerRows(req.file);
    const operations = [];
    const skippedRows = [];

    rows.forEach((row, index) => {
      const partner = buildActivationSupportPartnerFromRow(row);
    ...
    if (!operations.length) {
      return res.status(400).json({
        message: "No valid activation support partner rows found in CSV.",
        skippedRows,
      });
    }

    const result = await ActivationSupportPartner.bulkWrite(operations, {
      ordered: false,
    });

    return res.status(201).json({
      message: "Activation support partners imported successfully",
...
export const getActivationSupportPartners = async (req, res, next) => {
  try {
    ...
    const partners = await ActivationSupportPartner.find(query)
      .sort({ continent: 1, country: 1, destination: 1, company: 1 })
      .lean();

    return res.status(200).json({
      message: "Activation support partners fetched successfully",
      data: partners,
    });
  } catch (error) {
    return next(error);
...
export const getActivationSupportPartnerById = async (req, res, next) => {
  try {
    const partner = await ActivationSupportPartner.findById(
    ...
    if (!partner) {
      return res
        .status(404)
        .json({ message: "Activation support partner not found" });
    }

    return res.status(200).json({
      message: "Activation support partner fetched successfully",
      data: partner,
    });
  } catch (error) {
    return next(error);
...
export const updateActivationSupportPartner = async (req, res, next) => {
  try {
    const payload = await buildEditableActivationSupportPartnerPayload(req.body);
    const partner = await ActivationSupportPartner.findByIdAndUpdate(
      req.params.partnerId,
      { $set: payload },
    ...
    return res.status(200).json({
      message: "Activation support partner updated successfully",
      data: partner,
    });
  } catch (error) {
    ...
    if (error?.code === 11000) {
      return res.status(409).json({
        message:
          "An activation support partner with the same country, destination, company, email, and contact already exists.",
      });
    }
