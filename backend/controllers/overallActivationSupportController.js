...
const overallActivationSupportSchema = yup.object({
    ...
    nationalityOnPassport: yup.string().required("Nationality on passport is required"),
...
export const createOverallActivationSupport = async (req, res, next) => {
    try {
        ...
        const overallActivationSupport = await OverallActivationSupport.create(payload);
