...
import WorkationSupportPartner from "../models/WorkationSupportPartner.js";
...
const readWorkationSupportPartnerRows = (file) =>
...
const buildWorkationSupportPartnerFromRow = (row) => {
  ...
  return {
    ...
    country,
    destination,
    numberOfPeople: parseNumber(
      getRowValue(row, ["Number Of People", "Number of People"]),
    ),
    company,
...
const buildEditableWorkationSupportPartnerPayload = async (body = {}) => {
  const payload = await editableWorkationSupportPartnerSchema.validate(body, {
    abortEarly: false,
    stripUnknown: true,
  });
...
export const importWorkationSupportPartnersCsv = async (req, res, next) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        message:
          "Please upload a CSV file using form-data field workation-support-partners-file.",
      });
    }

    const rows = await readWorkationSupportPartnerRows(req.file);
    const operations = [];
    const skippedRows = [];

    rows.forEach((row, index) => {
      const partner = buildWorkationSupportPartnerFromRow(row);
    ...
    if (!operations.length) {
      return res.status(400).json({
        message: "No valid workation support partner rows found in CSV.",
        skippedRows,
      });
    }

    const result = await WorkationSupportPartner.bulkWrite(operations, {
      ordered: false,
    });

    return res.status(201).json({
      message: "Workation support partners imported successfully",
...
export const getWorkationSupportPartners = async (req, res, next) => {
  try {
    ...
    const partners = await WorkationSupportPartner.find(query)
      .sort({ continent: 1, country: 1, destination: 1, company: 1 })
      .lean();

    return res.status(200).json({
      message: "Workation support partners fetched successfully",
      data: partners,
    });
  } catch (error) {
    return next(error);
...
export const getWorkationSupportPartnerById = async (req, res, next) => {
  try {
    const partner = await WorkationSupportPartner.findById(
    ...
    if (!partner) {
      return res
        .status(404)
        .json({ message: "Workation support partner not found" });
    }

    return res.status(200).json({
      message: "Workation support partner fetched successfully",
      data: partner,
    });
  } catch (error) {
    return next(error);
...
export const updateWorkationSupportPartner = async (req, res, next) => {
  try {
    const payload = await buildEditableWorkationSupportPartnerPayload(req.body);
    const partner = await WorkationSupportPartner.findByIdAndUpdate(
      req.params.partnerId,
      { $set: payload },
    ...
    return res.status(200).json({
      message: "Workation support partner updated successfully",
      data: partner,
    });
  } catch (error) {
    ...
    if (error?.code === 11000) {
      return res.status(409).json({
        message:
          "A workation support partner with the same country, destination, company, email, and contact already exists.",
      });
    }
